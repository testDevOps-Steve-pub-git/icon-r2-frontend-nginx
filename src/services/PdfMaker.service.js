...
/* Private ********************************************************************/
  ...
  function isAgentWithoutTrade (immunization) {
    ...
    /**
     * Obscures all characters in an ID number, except the last 4.
     * @param {string} id - the ID to obscure (example: 'ABC0001234')
     * @returns {string} the obscured ID (example: '******1234')
     */
  function obscureDigits (id = `N/A`) {
    const VISIBLE_CHARACTERS = id.substring(id.length - 4)
    ...
    if (data.patientHcn) {
      demographicsList.ul.push({
        text: [
            ...
            { text: obscureDigits(data.patientHcn), bold: true }
        ]
      })
    }
    ...
    if (data.patientOiid) {
      demographicsList.ul.push({
        text: [
            ...
            { text: obscureDigits(data.patientOiid), bold: true }
        ]
      })
    }
  ...
  function appendImageFilesAsBase64 ({ text, data }) {
