...
/* @ngInject */
function immunizationLotInput$ctrl (Endpoint, Lot) {
  let $ctrl = this
      ...
      Endpoint.lookupLots($ctrl.snomed)
        ...
        .then(function (results) {
          ...
          results.forEach((lot) => {
            ...
            if (nrCheck === 'N-') { lot.lotDisplayNumber = lot.number.slice(2, lot.number.length) } else { lot.lotDisplayNumber = lot.number }
          })
          $ctrl.lotArray = results
          return results
        })
...
export default {
  name: 'immunizationLotInput',
  component: {
    bindings: {
      immunization: '=',
      snomed: '<'
    },
    controller: immunizationLotInput$ctrl,
